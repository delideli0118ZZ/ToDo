...
# -PostgresSQL을 비동기 방식으로 연결하여 사용할 수 있도록 구성함
# ---------------------------------------------------------
...
# create_async_engine:
#   - PostgresSQL과 연결한 후 데이터를 조회하거나 저장할 때 사용하는 세션
#   - DB에 연결한 후 데이터를 조회하거나 저장할 때 사용하는 세션
# AsyncSession:
#   - DB에 연결한 후 데이터를 조회하거나 저장할 때 사용하는 세션
#   - 비동기 방식으로 작동하는 세션이다.
...
# ---------------------------------------------------------
# [1]PostgresSQL에 연결할 주소 설정 (DB 접속 정보)
...
# ---------------------------------------------------------
# [3] 세션(session) 설정
# - 세션은DB와 데이터를 주고받을 수 있게 도와주는 통로이다.
# - autocommit=False : 성능 향상을 위해 자동 바녕하지 않음
...
# - 이후에 만들 모든 테이블 클래스들은 Base를 상속받아 정의한다.
...
# ---------------------------------------------------------
# [5] FastAPI에서 시용할 DB 세션 생성 함수
# - get_db() 함수는 `의존성 주입(Dependency Injection)`에 사용된다.
# - 즉, 우리가 직접 세션을 만들지 않아도,
#   FastAPI가 이 함수를 실행헤서 필요한 DB 세션을 자동으로 넣어준다.
